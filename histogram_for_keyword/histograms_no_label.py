#Generates keywords features with labels removed in the form of histograms
...
centroids = np.load('train_kmeans_centroids_no_label.npy')
if unique:
    centroids = np.load('train_kmeans_centroids_no_label_unique.npy') 
    ...
    with open(output_path+'{}.csv'.format(a), 'w') as f:
        for i in range(a*int(len(videos)/step),(a+1)*int(len(videos)/step)):
            ...
            if (len(videos[i]) > 1):
                ...
                if (i % 10000) == 0:
                    print(len(video_keywords)) #should be lower than the previous printed number
                try:
                    if type(videos[i][0]) == list:
                        d_labels = real_labels[videos[i][0][0]]
                    else:
                        d_labels = real_labels[videos[i][0]]
                    #checking is some labels are in keywords
                    for v in range(len(d_labels)):
                        label_is_in_keywords = 1
                        for w in range(len(d_labels[v])):
                            if not(d_labels[v][w].lower() in video_keywords):
                                label_is_in_keywords = 0
                                break
                        #removing these labels
                        if (label_is_in_keywords == 1):
                            for w in range(len(d_labels[v])):
                                if d_labels[v][w].lower() in video_keywords:
                                    video_keywords.remove(d_labels[v][w].lower())
                    if (i % 10000) == 0:
                        print(len(video_keywords)) #should be lower than the previous printed number
                except KeyError:
                    #print('No label for this video')
                    continue
